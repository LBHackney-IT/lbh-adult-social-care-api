...
Get-ChildItem -Path $PSScriptRoot -File -Recurse -exclude *.ps1 | % {
    ...
    if ($contents -match "lbh_adult_social_care_api") {
        $contents -replace 'lbh_adult_social_care_api', $apiName | Set-Content $_.PSPath
        Write-Host $("'{0}': contents changed." -f $fileName)
    }

    if ($fileName -match 'lbh_adult_social_care_api') {
        $newName = $_.Name -replace 'lbh_adult_social_care_api', $apiName
...
Where-Object { $_.Name -match 'lbh_adult_social_care_api' } | % {
    Write-Host $("Editing directory: '{0}'." -f $_.FullName)
    $newDirName = $_.Name -replace 'lbh_adult_social_care_api', $apiName
